...
- Translate upstream `build-config.ts` or esbuild build changes into the local package's `build-config.ts` metadata and shared `createRslibConfig` semantics or package options rather than copying them literally. Check entries, platform and syntax targets, externals, dts, shims, chunks, and defines as applicable.
