...
Install `storybook-next-rsbuild` and `next-rspack` together — the framework needs `next-rspack` in your project to obtain Next.js's build configuration. Keep `next-rspack` on the exact same version as `next`.
