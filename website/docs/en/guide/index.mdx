...
| [storybook-addon-rstack](https://github.com/rstackjs/storybook-rsbuild/tree/main/packages/addon-rstack)                       | Addon that reuses Rstack configuration inside Storybook    |
...
**`storybook`** <PeerRange name="storybook" /> — The range is pinned to the `storybook` peer of the bundled `@storybook/*` packages and is bumped alongside Storybook releases. Starting with v3.6.0, the minor version of Storybook Rsbuild tracks the Storybook minor it is built against: 3.6.x targets Storybook 10.6, 3.7.x will target Storybook 10.7, and so on. Upgrade guidance is available in the [Storybook release notes](https://storybook.js.org/blog/storybook-10/).
...
Reuse an app or library definition from your Rstack configuration inside Storybook.

See [integrating with Rstack](/guide/integrations/rstack).
