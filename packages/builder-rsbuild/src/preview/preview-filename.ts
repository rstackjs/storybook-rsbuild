...
function previewChunkFilename({ chunk }: Rspack.PathData, suffix: string) {
  ...
  const maxNameBytes =
    MAX_FILENAME_LENGTH - Buffer.byteLength(hashSuffix + suffix)
  const prefix =
    truncateToBytes(sanitizedName, maxNameBytes).replace(/[ .]+$/g, '') ||
    'chunk'

  return `${prefix}${hashSuffix}${suffix}`
}

export const developmentPreviewChunkFilename = (pathData: Rspack.PathData) =>
  previewChunkFilename(pathData, DEVELOPMENT_SUFFIX)

export const productionPreviewChunkFilename = (pathData: Rspack.PathData) =>
