...
| `process.env.RSPACK_CONFIG_VALIDATE ??= 'loose'` | Next.js emits config with webpack-only keys Rspack's schema rejects, so validation can't be `'strict'`. **Process-wide and load-bearing at compile time**: it also governs validation of the FINAL Storybook/Rsbuild config — including the user's own `webpackFinal`/`tools.rspack` mutations — when Rsbuild calls `rspack(finalConfig)`, not just extraction. Default is the loosest **non-silent** mode (`'loose'` prints validation issues, incl. user config typos, as warnings without throwing — unlike `'loose-silent'`). Only defaulted when unset, so a user value (`'strict'` to debug) wins. Read only by `@rspack/core` ≤ 1.5.x (Next 15 rows) | Already met on Next 16 rows (rspack 1.6.0 removed JS-side validation); on Next 15 rows, when Rspack loosens its validator or Next.js stops emitting webpack-only keys |
...
- **`@rspack/core` invariant**: `src/utils/check-rspack-invariant.ts` runs at the start of `rsbuildFinal` and aborts with the matrix link when `@rsbuild/core`'s and `next-rspack`'s `@rspack/core` resolve to different files. Otherwise plugin instances bound to one `NormalModule` class would hook a compilation that uses another — failures appear as inscrutable runtime errors far from the root cause. It reports **two distinct failure shapes** with different fixes: *version mismatch* (versions differ → realign via the matrix / override the version) and *duplicate physical copies* (versions equal, paths differ → a package-manager peer split, e.g. yarn Berry virtual packages over `@rspack/core`'s optional `@swc/helpers` peer → pin the splitting peer or `dedupe`; changing the `@rspack/core` version is a no-op there). Sandbox `package.json` exact-pins `@rsbuild/core` (not `^`) so the resolver cannot drift past the matrix entry.
