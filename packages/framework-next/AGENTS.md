...
- **HMR**: split, three pieces. Keep Next.js's `ReactRefreshRspackPlugin` (provides `$ReactRefreshRuntime$`) and add `ReactRefreshInitPlugin` for the `injectIntoGlobalHook()` bootstrap Next.js expects a separate entry to do. The third piece is the per-module footer (`module.hot.accept` self-accept): Fast Refresh only preserves state if it reaches story modules, so `buildNextLoaderChain` must select the rule Next.js paired with `builtin:react-refresh-loader`, not the refresh-less `issuerLayer: 'api-node'` rule. Without the footer, SWC's `$RefreshReg$`/`$RefreshSig$` calls bind to the no-op globals in `react-refresh-entry.cjs` and edits remount the component.
...
| save/restore `process.env.RSPACK_BINDING` around extraction | Importing `next-rspack/rspack-core` sets this process-wide to Next.js's forked binding. If it leaks, Rsbuild's standard Rspack wrappers hand their native plugin objects to the forked compiler and fail with `Class for Type … not found` | `@next/rspack-core` stops mutating `RSPACK_BINDING`, or Rspack scopes binding selection to each core instance |
| `delete process.env.BUILTIN_SWC_LOADER` (around `getBaseWebpackConfig`) | When set (a next-rspack perf knob), Next emits `builtin:next-swc-loader` instead of the JS `next-swc-loader`. The builtin has a different options schema, panics on standard `@rspack/core`, and `buildNextLoaderChain` can't shim it — so the bridge would silently degrade to Rsbuild's built-in SWC. We scrub it for the extraction (save/restore) to force the JS loader branch | Next drops the `BUILTIN_SWC_LOADER` escape hatch, or standard `@rspack/core` registers the builtin loader |
| `process.env.__NEXT_PRIVATE_RENDER_WORKER = 'defined'` | Makes `loadConfig()` **skip `loadWebpackHook()`**, which otherwise installs ~40 process-wide require-hook aliases remapping `webpack`/`webpack-sources`/`@babel/runtime`/… to `next/dist/compiled/*` — hijacking those `require`s for the entire Storybook process (incl. user `webpackFinal`/`webpackAddons` code that does `require('webpack')`). It suppresses the alias *registration*, not a throw (that only happens in standalone/untraced installs) | Next.js stops installing require-hook aliases from `loadConfig()`, or scopes its `_resolveFilename` patch to Next-internal parents |
...
| `NoopTraceSpanPlugin` | Next.js loaders such as `next-font-loader` and its CSS loaders read `this.currentTraceSpan`; `RspackProfilingPlugin` would set it but resolves `NormalModule` from Next.js's vendored rspack-core (distinct class from Rsbuild's rspack instance) | Next.js decouples tracing from `NormalModule` identity |
| `ReactRefreshInitPlugin` | Next.js's `ReactRefreshRspackPlugin` only wires `$ReactRefreshRuntime$`; the `injectIntoGlobalHook()` bootstrap normally lives in a separate entry Next.js injects via its own orchestration. We deliberately use Rspack's documented `EntryPlugin({ name: undefined })` global-entry contract, which injects the bootstrap into every regular and async entry. | Next.js moves the bootstrap into the plugin or exposes a helper |
...
| `next-image-mock.js` | `next/image` expects a `/_next/image` optimization endpoint. Load-bearing: `preset.ts` strips a user `next.config.webpack()` `next/image`(`$`) alias (with a warn) and spreads `STORYBOOK_OVERRIDE_ALIASES` FIRST, so a user alias can't silently defeat the mock by key-override or rspack insertion order | Storybook gains an image-optimization dev endpoint |
...
| `readProvidedMap` reading `DefinePlugin`/`ProvidePlugin` definitions via rspack's internal `._args[0]` (no public getter; webpack's `.definitions` is the other arm) | extracting Next.js's `DefinePlugin` (`__NEXT_*` defines) and `ProvidePlugin` keys requires reading the constructor arg rspack stashes on `._args[0]`; there is no public API. A wrapper-shape change yields `{}` silently → missing defines surface only as a render-time `ReferenceError`. **Both** arms warn when the map can't be read: `doExtract` for the `DefinePlugin` extraction, and `dedupProvidePluginKeys` (via an `onUnreadable` callback) for the `ProvidePlugin` dedup | rspack exposes a public definitions accessor on `DefinePlugin`/`ProvidePlugin` |
...
| `styled-jsx` alias pointing at resolved dir | dual-package resolution + singletons across `styled-jsx` / `styled-jsx/style`. Load-bearing like `next-image-mock`: `preset.ts` strips a user `next.config.webpack()` `styled-jsx`(`/…`,`$`) alias (with a warn) and drops Next.js's own `styled-jsx$`/`styled-jsx/style$` base entries so `STORYBOOK_OVERRIDE_ALIASES` is the single styled-jsx identity source | styled-jsx ships a modern `exports` map |
...
- **Injected dep sync**: `storybook-builder-rsbuild` is consumed via `dependenciesMeta.injected: true` because this package pins `@rsbuild/core@2.0.x` while siblings use `2.1.x`. After rebuilding the builder, you **must** run `pnpm install` at the repo root to re-sync the injected artifact; `pnpm build` alone won't propagate.
...
- **`@rsbuild/core` is a peer, not a dep**: declared as `peerDependencies: ^2.0.0`; consumers install exactly `2.0.7` from the version matrix. That release declares `@rspack/core@~2.0.4`, while `next-rspack@16.3.x` pins exactly `2.0.4` through `@next/rspack-core`, so consumers must also add the matrix's scoped override. A wide direct dependency range would let package managers drift to a mismatched Rspack release.
- **`@rspack/core` invariant**: `src/utils/check-rspack-invariant.ts` runs at the start of `rsbuildFinal` and aborts with the matrix link when `@rsbuild/core`'s and `next-rspack → @next/rspack-core`'s `@rspack/core` resolve to different files. Otherwise plugin instances bound to one `NormalModule` class would hook a compilation that uses another — failures appear as inscrutable runtime errors far from the root cause. It reports **three failure shapes** with different fixes: *cross-major mismatch* (Next.js <=16.2 / Rspack 1 → upgrade `next` and `next-rspack` to >=16.3), *version mismatch* (versions differ → install `@rsbuild/core@2.0.7` with the scoped `@rspack/core@2.0.4` override), and *duplicate physical copies* (versions equal, paths differ → a package-manager peer split, e.g. yarn Berry virtual packages over `@rspack/core`'s optional `@swc/helpers` peer → pin the splitting peer or `dedupe`; changing the `@rspack/core` version is a no-op there). Sandbox `package.json` exact-pins `@rsbuild/core` (not `^`) so the resolver cannot drift past the matrix entry.
