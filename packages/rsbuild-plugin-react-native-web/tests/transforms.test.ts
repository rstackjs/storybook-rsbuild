...
describe('transformReanimatedWebUtils reanimated 4.x (plain ESM webUtils.js)', () => {
  const webUtils4Path =
    'node_modules/react-native-reanimated/lib/module/ReanimatedModule/js-reanimated/webUtils.js'

  const esmCode = `'use strict';
...
export const createReactDOMStyle = createReactDOMStyleModule.default;
export const createTransformValue = preprocessModule.createTransformValue;
`

  const resolveModule = (modulePath: string) =>
    modulePath.startsWith('react-native-web/')
      ? `/abs/node_modules/${modulePath}`
      : modulePath

  it('rewrites bare react-native-web specifiers to absolute paths', () => {
    const result = transformReanimatedWebUtils(esmCode, webUtils4Path, true, {
      resolveModule,
    })

    expect(result.changed).toBe(true)
    expect(result.code).toContain(
      "from '/abs/node_modules/react-native-web/dist/cjs/exports/StyleSheet/compiler/createReactDOMStyle.js'",
    )
    expect(result.code).toContain(
      "from '/abs/node_modules/react-native-web/dist/cjs/exports/StyleSheet/preprocess.js'",
    )
    expect(result.map).not.toBeNull()
  })

  it('returns 4.x code unchanged when resolveModule is not provided', () => {
    const result = transformReanimatedWebUtils(esmCode, webUtils4Path, true)

    expect(result.changed).toBe(false)
    expect(result.code).toBe(esmCode)
    expect(result.map).toBeNull()
  })

  it('returns 4.x code unchanged when resolveModule throws', () => {
    const result = transformReanimatedWebUtils(esmCode, webUtils4Path, true, {
      resolveModule: () => {
  ...
  it('returns 4.x code unchanged when it has no react-native-web imports', () => {
    const result = transformReanimatedWebUtils(
      'export const foo = 1;\n',
      webUtils4Path,
      true,
